...
-- Add the active-version foreign key safely for existing databases
DO $prompt_active_version_fk$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_prompt_definitions_active_version'
    ) THEN
    ...
    END IF;
END
$prompt_active_version_fk$;

-- 3. Activation and rollback logs
