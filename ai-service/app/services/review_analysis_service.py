...
DEFAULT_SENTIMENT_PROMPT_VERSION = "review-analysis:v1"
...
class ReviewAnalysisService:
    ...
    def _resolve_prompt_version(
        ...
        if request.promptVersion and request.promptVersion.strip():
            return request.promptVersion.strip()

        return DEFAULT_SENTIMENT_PROMPT_VERSION
    ...
    async def analyze(
        ...
        if mode == "local":
            ...
            # 纯本地模型没有实际使用 System Prompt。
            response.promptVersion = None
            return response

        if mode == "hybrid":
            response = await self._analyze_hybrid(
                request,
                analysis_version,
            )
        else:
        ...
        response.promptVersion = self._resolve_prompt_version(
            request
        )
        return response
    ...
    async def batch_analyze(
        ...
        if system_prompt and system_prompt.strip():
            prompt_updates["systemPrompt"] = system_prompt

        if prompt_version and prompt_version.strip():
            prompt_updates["promptVersion"] = (
                prompt_version.strip()
            )

        for req in requests:
            effective_request = (
                req.model_copy(update=prompt_updates)
                if prompt_updates
                else req
            )
